...
/** @typedef {{OperatorERID: string, Email: string, FullName: string}} ClaudeUser */

/**
 * @typedef {object} AIPRMClient
 * @property {string} APIEndpoint
 * @property {ClaudeUser} ClaudeUser
 * @property {User} User
...
const AIPRMClient = {
  ...
  // fetch the user profile from Claude session API endpoint
  async init() {
    const UserFootprint = '';
    ...
    return (
      fetch('/api/bootstrap', {
        ...
        // set the Claude user
        .then((res) => {
          // check if response has account property
          if (!res || !Object.prototype.hasOwnProperty.call(res, 'account')) {
            throw new Error('Account response is missing account property.');
          }

          // check if account property has uuid, email_address and full_name properties
          if (
            !res.account ||
            !Object.prototype.hasOwnProperty.call(res.account, 'uuid') ||
            !Object.prototype.hasOwnProperty.call(
              res.account,
              'email_address'
            ) ||
            !Object.prototype.hasOwnProperty.call(res.account, 'full_name')
          ) {
            throw new Error(
              'Account response is missing uuid, email_address or full_name property.'
            );
          }

          this.ClaudeUser = {
            OperatorERID: res.account.uuid,
            Email: res.account.email_address,
            FullName: res.account.full_name,
          };

          console.log('Claude User:', this.ClaudeUser);

          const OperatorERID = this.ClaudeUser.OperatorERID;
          ...
          this.User = {
            // Send the anonymous, not identifiable Claude user ID to AIPRM to link the user to his own prompts
            OperatorERID: OperatorERID,
            SystemNo: SystemNo.CLAUDEAI_CHAT,
  ...
  // unlink AIPRM user and account from Claude user
  unlinkUser() {
  ...
  linkUser() {
    ...
    // fetch using JWT token
    return this.fetchWithToken(`${APIEndpointAPP}/auth/claude/callback`, {
      method: 'POST',
      body: JSON.stringify({
