...
window.AIPRM = {
  ...
  async checkVersion() {
    try {
      const response = await this.fetch(
        `https://version.aiprm.com/check/claude/${VERSION}?v=${this.CacheBuster}`
      );
  ...
  async fetchStaticMessages() {
    try {
      const response = await this.fetch(
        `https://static.aiprm.com/claude/${VERSION}/messages.json?v=${this.CacheBuster}`
      );
  ...
  async init() {
    ...
    // Wait for prompt templates, lists, topics, activities, config from remote JSON file, languages and messages
    await Promise.all([
  ...
  setPromptPanelHintPlaceholder(inputField) {
    ...
    this.updatePromptTextareaPlaceholder(
      inputField,
      'Press / to search AIPRM prompts ...'
    );
  },

  /**
   * Update the placeholder text of the prompt textarea (using CSS variable)
   *
  ...
  updatePromptTextareaPlaceholder(inputField, placeholderText) {
    const placeholderElement = inputField.querySelector(
      this.Config.getSelectorConfig().PromptTextareaInputClass
    );
  ...
  insertPromptPanel() {
    ...
    const inputField = document.querySelector(
      this.Config.getSelectorConfig().PromptTextareaInput
    );
  ...
  async injectLiveCrawlingResult(sourceURL, message) {
    ...
    if (!this.Config.isLiveCrawlingEnabled()) {
      ...
      // remove CRAWLEDTEXT/CRAWLEDSOURCE placeholder from message (let Claude to guess at least using the URL)
      return message
...
`;
  ...
  async handleTokensEvent(event) {
    // check if user has Claude account - no linking possible for anonymous users (based on OperatorERID)
    if (
      !this.Client.User.OperatorERID.match(this.Config.getPatternOperatorERID())
    ) {
      this.showNotification(
        NotificationSeverity.WARNING,
        'Please log in or sign up for a Claude account first.',
        false
      );
    ...
    // link Claude user to AIPRM user and account
    try {
      await this.Client.linkUser();
    } catch (error) {
      this.showNotification(
        NotificationSeverity.ERROR,
        'Could not connect your Claude account to AIPRM, please try again later.',
        false
      );
    ...
    try {
      ...
      if (!this.Client.User.IsLinked) {
        this.showNotification(
          NotificationSeverity.INFO,
          'Connecting your Claude account to AIPRM, please wait...',
          false
        );
      }
  ...
  async fetchMessages(render = true) {
    try {
      ...
      console.error('Could not load messages. Error: ', error);
  ...
  isTemplateForModelOrGizmo(template, modelOrGizmoCode) {
    return template.ModelS?.find((model) => model === modelOrGizmoCode);
  },
  ...
  replaceFetch() {
    window.fetch = async (...t) => {
      ...
      if (config.EnableConversationMessagesModification) {
        const conversationID = t?.[0]
          ?.toString()
          ?.match(config.EndpointConversationChatMessages)?.[1];
        ...
        if (
          ...
          // console.log(
          //   'Intercepting chat messages request for current conversation',
          //   conversationID
          // );
      ...
      if (!t[0] || !t[0]?.toString()?.match(EndpointConversation)) {
        return this.fetch(...t);
      }

      // Reset the prompt submit flag
      this.IsPromptSubmit = false;
      ...
      if (
        ...
        if (!this.SelectedContinueActionPrompt) {
          this.Client.usePrompt(0, UsageTypeNo.SEND);
        }
      ...
      if (template || !this.SelectedContinueActionPrompt) {
        this.Client.usePrompt(template ? template.ID : 0, UsageTypeNo.SEND);
      }
      ...
      try {
        if (template) {
          ...
          body.prompt = promptTextUpdated;

          // Replace Live Crawling placeholders with the Live Crawling result
          body.prompt = await this.injectLiveCrawlingResult(
            prompt,
            body.prompt
          );
        }
        ...
        useAuxIndexLookup =
          this.Client.UserQuota.hasCustomIndexesFeatureEnabled() &&
          body.prompt.match(AuxIndexLookupDefinition);
        ...
        if (useAuxIndexLookup) {
          ...
          if (!augmentedPrompt?.Prompt) {
            ...
            body.prompt = augmentedPrompt.Prompt;
      ...
      .forEach((buttonsDiv) => {
        ...
        if (!watermarkToggle) {
          ...
          watermarkToggle.className =
            'AIPRM__watermarkToggle AIPRM__rounded-lg AIPRM__p-1.5 hover:AIPRM__bg-bg-300 hover:AIPRM__text-gray-700 dark:AIPRM__text-gray-400 dark:hover:AIPRM__bg-dark-bg-300 dark:hover:AIPRM__text-gray-200 AIPRM__cursor-pointer AIPRM__flex AIPRM__flew-row AIPRM__items-center';
          watermarkToggle.title = 'Hide AIPRM watermark';
          watermarkToggle.addEventListener(
            'click',
            this.toggleWatermarkInline.bind(this)
          );
          watermarkToggle.innerHTML = svg('EyeSlashSmall');
  ...
  async handleElementAdded(e) {
    ...
    // Ignore excessive re-rendering of the same element (e.g. "Start new chat" button icon)
    if (
      e.classList?.toString() &&
      selectorConfig.ElementAddedIgnore.includes(e.classList?.toString())
    ) {
      return;
    }
    ...
    if (
      ...
      e.classList?.toString() === selectorConfig.ElementAddedSidebarClassList
    ) {
      this.setupSidebar();
      return;
    }
    ...
    if (
      document.querySelector(selectorConfig.ElementAddedExportButtonDisable)
    ) {
    ...
    if (
      document.querySelector(selectorConfig.ElementAddedPromptTemplatesSidebar)
    ) {
      this.insertPromptTemplatesSidebar();
    }

    if (document.querySelector(selectorConfig.PromptSubmitButton)) {
      // Identify prompt submit on dashboard using button click or Enter key
      this.identifyPromptSubmit();
    }

    // Find the "New chat" widget buttons and override them to navigate to /new
    this.overrideNewChatWidget();

    // Update prompt panel placeholder when the textarea was (re-)added
    if (
      e.classList
        ?.toString()
        ?.includes(selectorConfig.ElementAddedPromptTextarea)
    ) {
  ...
  // Override "New chat" widget buttons to navigate to /new
  overrideNewChatWidget() {
    if (!this.Config.getPromptTemplatesConfig().OverrideNewChatWidget) {
      return;
    }

    // There are multiple buttons depending on the viewport
    const newChatButtons = document.querySelectorAll(
      this.Config.getSelectorConfig().NewChatWidgetButtons
    );

    // If there are no new chat buttons, return
    if (!newChatButtons) {
      return;
    }

    // Add AIPRM class to the new chat buttons and override the click event
    newChatButtons?.forEach((newChatButton) => {
      newChatButton?.classList.add('AIPRM__new-chat-button');
  ...
  promptSubmitButtonListener() {
    if (
      window.location.pathname &&
      !window.location.pathname.match(
        window.AIPRM.Config.getPromptTemplatesConfig()
          .PromptSubmitPathnamePattern
      )
    ) {
      return;
    }

    window.AIPRM.IsPromptSubmit = true;
  },

  promptTextareaInputListener(event) {
    if (
      window.location.pathname &&
      !window.location.pathname.match(
        window.AIPRM.Config.getPromptTemplatesConfig()
  ...
  // Identify prompt submit on dashboard using button click or Enter key
  identifyPromptSubmit() {
    const selectorConfig = this.Config.getSelectorConfig();

    const button = document.querySelector(selectorConfig.PromptSubmitButton);

    button?.removeEventListener('click', this.promptSubmitButtonListener);
    button?.addEventListener('click', this.promptSubmitButtonListener);

    const input = document.querySelector(selectorConfig.PromptTextareaInput);

    input?.removeEventListener('keydown', this.promptTextareaInputListener);
    input?.addEventListener('keydown', this.promptTextareaInputListener);
  },
  ...
  insertPromptTemplatesSidebar() {
    ...
    if (
      !document.querySelector(selectorConfig.ChatLogContainer) &&
    ...
    sidebarIcon.className =
      'AIPRM__p-2 AIPRM__items-center AIPRM__transition-colors AIPRM__duration-200 AIPRM__cursor-pointer AIPRM__text-sm AIPRM__rounded-md AIPRM__border AIPRM__bg-bg-300 dark:AIPRM__bg-dark-bg-200 AIPRM__border-black/10 dark:AIPRM__border-white/20 hover:AIPRM__bg-bg-400 dark:hover:AIPRM__bg-dark-bg-300';
    ...
    sidebarIcon.addEventListener('click', () => {
      const claudeSidebar = document.querySelector(
        selectorConfig.ClaudeSidebar
      );
      if (claudeSidebar) {
        claudeSidebar.querySelector(selectorConfig.ClaudeSidebarClose)?.click();
      }

      const sidebar = document.getElementById('AIPRM__sidebar');
      // animate opening and closing of sidebar
      sidebar.classList.toggle('AIPRM__hidden');
    ...
    sidebar.className = 'AIPRM__hidden AIPRM__absolute';

    sidebar.innerHTML = /*html*/ `
    <div class="3xl:AIPRM__w-[85rem] 2xl:AIPRM__w-[50rem] lg:AIPRM__w-[35rem] md:AIPRM__w-96"></div>
    <div class="3xl:AIPRM__w-[85rem] 2xl:AIPRM__w-[50rem] lg:AIPRM__w-[35rem] md:AIPRM__w-96 AIPRM__fixed AIPRM__bottom-0 AIPRM__top-0 AIPRM__flex AIPRM__w-full AIPRM__flex-col AIPRM__z-[15] AIPRM__right-0 AIPRM__mt-16 md:AIPRM__pb-4 md:AIPRM__pr-1 AIPRM__mr-3">
      <div class="AIPRM__border AIPRM__flex-1 AIPRM__overflow-hidden AIPRM__rounded-xl AIPRM__shadow-lg AIPRM__bg-bg-100 AIPRM__text-gray-700 dark:AIPRM__bg-dark-bg-100 dark:AIPRM__text-white dark:AIPRM__shadow-gray-600/50">
        <div class="AIPRM__flex AIPRM__h-full AIPRM__flex-col">
          <div class="AIPRM__overflow-x-visible AIPRM__overflow-y-auto AIPRM__scroll-pb-6 AIPRM__min-h-[0px] AIPRM__overscroll-contain">
            <div class="AIPRM__px-5 AIPRM__pb-4 AIPRM__pt-3">
              <div id="AIPRM__sidebar-container" class="AIPRM__flex AIPRM__items-center">
                <h1 id="AIPRM__sidebar-container-h" class="AIPRM__hidden"></h1>
              </div>
            </div>
          </div>
        </div>
    ...
    document
      .querySelector(selectorConfig.SidebarButton)
  ...
  insertSavePromptAsTemplateButton() {
    ...
    const buttons = firstPrompt.querySelectorAll(
    ...
    saveButton.className =
      'save-prompt-button AIPRM__p-2 AIPRM__py-1.5 AIPRM__flex AIPRM__flex-row AIPRM__items-center AIPRM__rounded-md hover:AIPRM__bg-bg-400 hover:AIPRM__text-gray-700 dark:AIPRM__text-gray-400 dark:hover:AIPRM__bg-dark-bg-400 dark:hover:AIPRM__text-gray-200 disabled:dark:hover:AIPRM__text-gray-400 AIPRM__text-sm AIPRM__flex AIPRM__gap-1';

    saveButton.title = 'Save prompt as template';
    saveButton.addEventListener('click', this.createNewPrompt.bind(this));
    saveButton.innerHTML = svg('Save') + ' Save';
  ...
  /**
   * Fix prompt template placehoders
   *
   * @param {Prompt} prompt
   */
  fixPlaceholders(prompt) {
    const config = this.Config.getPromptTemplatesConfig();

    if (config.FixPlaceholders) {
  ...
  async createNewPrompt(e) {
    ...
    form.elements['AuthorName'].value =
      this.Client.AppUser?.FullName || this.Client.ClaudeUser?.FullName || '';
  ...
  async showSavePromptModal(e, promptPlugins) {
    ...
    if (
      ...
      if (prompt) {
        promptTemplate = prompt.innerText;
      } else {
                  ...
                  <div id="AIPRM__PromptForm" class="${
                    ...
                    <div class="AIPRM__flex">
                      <div class="AIPRM__mr-4 AIPRM__w-full">
                        <label>Topic</label>
                        <select name="Topic"
                          class="AIPRM__select AIPRM__mt-2 AIPRM__mb-3 AIPRM__bg-gray-100 dark:AIPRM__bg-gray-850 dark:AIPRM__border-gray-700 dark:hover:AIPRM__bg-gray-800 AIPRM__rounded AIPRM__w-full AIPRM__px-3 AIPRM__py-2"
                          ${isAdvancedMode ? ' required ' : ''}>
                          ${this.Topics.map(
                            (topic) => /*html*/ `
                    ...
                    <div class="AIPRM__flex">
                      ...
                      <div class="AIPRM__w-full">
                        ...
                        <select multiple multiselect-max-items="1" multiselect-hide-x="true" name="ModelS"
                          class="AIPRM__select AIPRM__mt-2 AIPRM__mb-3 AIPRM__bg-gray-100 dark:AIPRM__bg-gray-850 dark:AIPRM__border-gray-700 dark:hover:AIPRM__bg-gray-800 AIPRM__rounded AIPRM__w-full">
  ...
  buildBasicPrompt() {
    ...
    if (currentPromptTypeNo === PromptTypeNo.TEAM) {
      // Use AIPRM user full name if available, otherwise Claude user name as fallback
      form.elements['AuthorName'].value =
        this.Client.AppUser?.FullName || this.Client.ClaudeUser?.FullName || '';
    }
  ...
  addExportButton() {
    ...
    buttons.className = 'AIPRM__py-4';

    // Create the "Export Button" element
    const button = document.createElement('a');
    button.id = 'export-button';
    button.className = css`SidebarButton`;
    button.innerHTML = /*html*/ `${svg`Export`} <span class="AIPRM__ml-2">Export Chat</span>`;
    ...
    // Append the "Export Button" to the fragment
    buttons.appendChild(button);

    // Create and insert the "Referral Button" element
    const referralsButton = this.Referrals.addSidebarButton();

    if (referralsButton) {
      buttons.appendChild(referralsButton);
    }

    // Create the "Version" element
    const version = document.createElement('a');
    version.id = 'AppName';
    version.className = css`SidebarButton`;
    version.innerHTML =
      /*html*/ `${svg`Rocket`} <span class="AIPRM__ml-2">` +
      AppName +
      ' powered</span>';
    version.href = AppURL;
    version.target = '_blank';
    version.title = AppName + ' powered';

    // Append the "Version" element to the fragment
    buttons.appendChild(version);

    // Create the "AIPRM Community Forum" element
    const forum = document.createElement('a');
    forum.className = css`SidebarButton`;
    forum.innerHTML = `${svg(
      'Topic'
    )} <span class="AIPRM__ml-2">AIPRM Community Forum</span>`;
    forum.href = AppCommunityForumURL;
    forum.target = '_blank';
    forum.title = 'AIPRM Community Forum';

    // Append the "AIPRM Community Forum" element to the fragment
    buttons.appendChild(forum);

    // Append the buttons to the fragment
    fragment.appendChild(buttons);

    // Get the account button as a reference
    const accountButton = document.querySelector(
      selectorConfig.AccountSidebarButton
    );

    if (!accountButton) {
      console.error('addExportButton: No account button found');
      return;
    }

    // Insert the fragment before the account button
    accountButton.parentElement.insertBefore(fragment, accountButton);
  },
  ...
  getNewChatButtons() {
    ...
    // Get the "New Chat" buttons in the sidebar
    const newChatButtons = [
      ...(sidebar?.querySelectorAll(selectorConfig.NewChatSidebarButtons) ??
        []),
    ].filter(
      (e) =>
        e.innerText === selectorConfig.NewChatSidebarButtonText ||
        e.href?.includes('/new')
    );
    ...
    return [...newChatButtons, AddButton].filter((button) => button);
  },
  ...
  async insertPromptTemplatesSection() {
    ...
    // Hide also "upgrade please"
    const upgrade = document.querySelector(
      selectorConfig.DashboardTitleUpgrade
    );

    if (upgrade) {
      upgrade.classList.add('AIPRM__hidden');
    }
    ...
    let templatesWithGizmoStarterPrompts = [...templates];
    ...
    <div class="AIPRM__flex AIPRM__flex-1 AIPRM__gap-3.5 AIPRM__justify-between AIPRM__items-center AIPRM__flex-col sm:AIPRM__flex-row AIPRM__mt-6">
      <div class="AIPRM__text-left" style="margin-top: -1rem;">
        ...
        <select class="AIPRM__select AIPRM__bg-bg-300 AIPRM__border-0 AIPRM__text-sm AIPRM__rounded AIPRM__block AIPRM__w-full dark:AIPRM__bg-dark-bg-200 dark:AIPRM__border-gray-600 dark:AIPRM__placeholder-gray-400 dark:AIPRM__text-white hover:AIPRM__bg-bg-400 focus:AIPRM__ring-0 dark:hover:AIPRM__bg-dark-bg-300 pageSizeSelect AIPRM__px-3 AIPRM__py-2">
          ${pageSizeOptions
            .map(
              (pageSize) => /*html*/ `
  ...
  `;
    ...
    <div class="${css`column`} AIPRM__relative" style="min-width: 0">
      ...
      <ul class="AIPRM__flex AIPRM__flex-col AIPRM__gap-3.5 AIPRM__mb-4">
        <div class="md:AIPRM__hidden AIPRM__text-left AIPRM__text-sm">
          ...
          <a href="#" title="View / hide lists" 
            ...
            class="AIPRM__rounded AIPRM__flex AIPRM__justify-center AIPRM__items-center AIPRM__p-2 AIPRM__text-gray-800 AIPRM__bg-bg-300 dark:AIPRM__bg-dark-bg-200 dark:AIPRM__border-gray-600 dark:AIPRM__placeholder-gray-400 dark:AIPRM__text-white hover:AIPRM__bg-bg-400 focus:AIPRM__ring-0 dark:hover:AIPRM__bg-dark-bg-300">
        ...
        <ul id="listsView" class="AIPRM__border-b AIPRM__border-gray-200 dark:AIPRM__border-gray-700 dark:AIPRM__text-gray-400 AIPRM__hidden md:AIPRM__flex AIPRM__flex-wrap AIPRM__font-medium AIPRM__text-center AIPRM__text-gray-500 AIPRM__text-sm AIPRM__whitespace-nowrap">
            ...
            <a href="#" id="favoritePromptsTab" title="Your &quot;Favorites&quot; List" 
              ...
              class="${
                ...
                this.PromptTemplatesList === favoritesList.ID
                  ? 'AIPRM__bg-bg-300 dark:AIPRM__bg-dark-bg-200'
                  : ''
              } dark:hover:AIPRM__bg-dark-bg-200 dark:hover:AIPRM__text-gray-300 hover:AIPRM__bg-bg-300 hover:AIPRM__text-gray-600 AIPRM__p-4 AIPRM__rounded-t-lg AIPRM__flex AIPRM__justify-center AIPRM__w-full">
          ...
          <li class="AIPRM__flex-1 AIPRM__mr-2">
            ...
            class="${
              this.PromptTemplatesType === PromptTemplatesType.PUBLIC
                ? 'AIPRM__bg-bg-300 dark:AIPRM__bg-dark-bg-200'
                : ''
            } dark:hover:AIPRM__bg-dark-bg-200 dark:hover:AIPRM__text-gray-300 hover:AIPRM__bg-bg-300 hover:AIPRM__text-gray-600 AIPRM__inline-block AIPRM__p-4 AIPRM__rounded-t-lg AIPRM__w-full">
              Public
            </a>
          </li>
          <li class="AIPRM__flex-1 AIPRM__mr-2">
                    ...
                    <li class="AIPRM__flex-1 AIPRM__mr-2">
                      ...
                      class="${
                        ...
                        this.PromptTemplatesList === list.ID
                          ? 'AIPRM__bg-bg-300 dark:AIPRM__bg-dark-bg-200'
                          : ''
                      } dark:hover:AIPRM__bg-dark-bg-200 dark:hover:AIPRM__text-gray-300 hover:AIPRM__bg-bg-300 hover:AIPRM__text-gray-600 AIPRM__p-4 AIPRM__rounded-t-lg AIPRM__flex AIPRM__justify-center AIPRM__items-center AIPRM__gap-2 AIPRM__w-full"> 
                      ${
                        list.ListTypeNo === ListTypeNo.TEAM_CUSTOM
          ...
          <li class="AIPRM__flex-1">
            ...
            class="AIPRM__rounded AIPRM__flex AIPRM__justify-center AIPRM__items-center AIPRM__m-2 AIPRM__p-2 AIPRM__font-medium AIPRM__text-gray-800 AIPRM__bg-bg-300 hover:AIPRM__bg-bg-400 dark:AIPRM__bg-dark-bg-200 dark:AIPRM__border-gray-600 dark:AIPRM__text-gray-400 dark:hover:AIPRM__text-white dark:hover:AIPRM__bg-dark-bg-300">
            ${svg('Plus')} &nbsp; Add List
            </a>
          </li>
        </ul>
    
        <div class="AIPRM__grid AIPRM__grid-cols-2 AIPRM__flex-row AIPRM__gap-3 AIPRM__items-end AIPRM__justify-between AIPRM__w-full md:last:AIPRM__mb-6 AIPRM__pt-2 AIPRM__stretch AIPRM__text-left AIPRM__text-sm ${
          !isSidebarView ? ' lg:AIPRM__flex ' : ' 2xl:AIPRM__flex '
        }">
          <div>
            <label for="topicSelect" class="AIPRM__block AIPRM__text-sm AIPRM__font-medium">Topic</label>
        
            <select id="topicSelect" class="AIPRM__select AIPRM__bg-bg-300 AIPRM__border-0 AIPRM__text-sm AIPRM__rounded AIPRM__block AIPRM__w-full dark:AIPRM__bg-dark-bg-200 dark:AIPRM__border-gray-600 dark:AIPRM__placeholder-gray-400 dark:AIPRM__text-white hover:AIPRM__bg-bg-400 focus:AIPRM__ring-0 dark:hover:AIPRM__bg-dark-bg-300 AIPRM__px-3 AIPRM__py-2">
          ...
          <div>
            ...
            <select id="sortBySelect" class="AIPRM__select AIPRM__bg-bg-300 AIPRM__border-0 AIPRM__text-sm AIPRM__rounded AIPRM__block AIPRM__w-full dark:AIPRM__bg-dark-bg-200 dark:AIPRM__border-gray-600 dark:AIPRM__placeholder-gray-400 dark:AIPRM__text-white hover:AIPRM__bg-bg-400 focus:AIPRM__ring-0 dark:hover:AIPRM__bg-dark-bg-300 AIPRM__px-3 AIPRM__py-2">
          ...
          <div>
            ...
            <select id="modelSelect" class="AIPRM__select AIPRM__bg-bg-300 AIPRM__border-0 AIPRM__text-sm AIPRM__rounded AIPRM__block AIPRM__w-full dark:AIPRM__bg-dark-bg-200 dark:AIPRM__border-gray-600 dark:AIPRM__placeholder-gray-400 dark:AIPRM__text-white hover:AIPRM__bg-bg-400 focus:AIPRM__ring-0 dark:hover:AIPRM__bg-dark-bg-300 AIPRM__px-3 AIPRM__py-2">
          ...
          <div class="AIPRM__whitespace-nowrap AIPRM__flex">
            <button title="Create New Prompt" 
              ...
              class="AIPRM__rounded AIPRM__justify-center AIPRM__items-center AIPRM__hidden  AIPRM__mr-1 AIPRM__p-2 AIPRM__px-2.5 AIPRM__font-medium AIPRM__text-gray-800 AIPRM__bg-bg-300 hover:AIPRM__bg-bg-400 dark:AIPRM__bg-dark-bg-200 dark:AIPRM__border-gray-600 dark:AIPRM__text-gray-400 dark:hover:AIPRM__text-white dark:hover:AIPRM__bg-dark-bg-300 ${
                !isSidebarView
                  ? ' lg:AIPRM__inline-block '
                  : ' 2xl:AIPRM__inline-block '
              }">
              ${svg('Plus')}
            </button>
            <input id="promptSearchInput" type="search" class="AIPRM__bg-bg-300 AIPRM__border-0 AIPRM__text-sm AIPRM__rounded AIPRM__inline-block AIPRM__w-full dark:AIPRM__bg-dark-bg-200 dark:AIPRM__border-gray-600 dark:AIPRM__placeholder-gray-400 dark:AIPRM__text-white hover:AIPRM__bg-bg-400 focus:AIPRM__ring-0 lg:AIPRM__w-[260px] dark:hover:AIPRM__bg-dark-bg-300 AIPRM__px-3 AIPRM__py-2" placeholder="Search" 
            value="${sanitizeInput(
              this.PromptSearch
          ...
          <div class="${
            !isSidebarView ? 'lg:AIPRM__hidden' : '2xl:AIPRM__hidden'
          }">
            <button title="Create New Prompt" 
              onclick="event.preventDefault(); AIPRM.createNewPrompt()" 
              class="AIPRM__rounded AIPRM__w-full AIPRM__flex AIPRM__justify-center AIPRM__items-center AIPRM__p-2 AIPRM__font-medium AIPRM__text-gray-800 AIPRM__bg-bg-300 hover:AIPRM__bg-bg-400 dark:AIPRM__bg-dark-bg-200 dark:AIPRM__border-gray-600 dark:AIPRM__text-gray-400 dark:hover:AIPRM__text-white dark:hover:AIPRM__bg-dark-bg-300">
              ${svg('Plus')} &nbsp; Add Prompt
            </button>
          </div>
        </div>
        ...
        <ul class="${css`ul`} AIPRM__grid AIPRM__grid-cols-1 ${
      !isSidebarView
        ? ' lg:AIPRM__grid-cols-2 2xl:AIPRM__grid-cols-4 '
        : ' 2xl:AIPRM__grid-cols-2 '
    }">
          ${currentTemplates
            .map(
              (template) => /*html*/ `
            <button onclick="${
              isSidebarView
                ? "document.getElementById('AIPRM__sidebar-icon').click();"
                : ''
            } AIPRM.selectPromptTemplateByIndex(${
                template.ID
                  ...
                  ? /*html*/ `
                    <div class="AIPRM__absolute AIPRM__top-0 AIPRM__left-0 AIPRM__flex AIPRM__text-gray-400 lg:AIPRM__invisible group-hover:AIPRM__visible">
                      <a title="${
                        ...
                        class="AIPRM__p-1 AIPRM__rounded-md hover:AIPRM__bg-bg-200 hover:AIPRM__text-gray-700 dark:AIPRM__text-gray-400 dark:hover:AIPRM__bg-dark-bg-100 dark:hover:AIPRM__text-gray-200 disabled:dark:hover:AIPRM__text-gray-400" onclick="event.stopPropagation(); ${
                          template.IsHidden
                  ...
                  ? /*html*/ `
                    <div class="AIPRM__absolute AIPRM__top-0 AIPRM__left-0 AIPRM__flex AIPRM__text-gray-400 lg:AIPRM__invisible group-hover:AIPRM__visible">
                      <a title="Remove this prompt from the list"
                        class="AIPRM__p-1 AIPRM__rounded-md hover:AIPRM__bg-bg-300 hover:AIPRM__text-gray-700 dark:AIPRM__text-gray-400 dark:hover:AIPRM__bg-dark-bg-100 dark:hover:AIPRM__text-gray-200 disabled:dark:hover:AIPRM__text-gray-400" onclick="event.stopPropagation(); AIPRM.removeFromList('${
                          this.PromptTemplatesList
                        }', ${template.ID})">
                        ${svg('Cross')}
                      </a>
              ...
              <div class="AIPRM__flex AIPRM__items-start AIPRM__w-full AIPRM__justify-between">
                ...
                <div class="AIPRM__flex AIPRM__gap-4 AIPRM__justify-center lg:AIPRM__gap-1 lg:AIPRM__pl-2 AIPRM__mt-1 AIPRM__right-2 AIPRM__text-gray-400 lg:AIPRM__invisible group-hover:AIPRM__visible">
                  ...
                  ${
                    !template.OwnPrompt
                      ? /*html*/ `
                      <a title="View prompt template source" class="AIPRM__p-1 AIPRM__rounded-md hover:AIPRM__bg-bg-200 hover:AIPRM__text-gray-700 dark:AIPRM__text-gray-400 dark:hover:AIPRM__bg-dark-bg-100 dark:hover:AIPRM__text-gray-200 disabled:dark:hover:AIPRM__text-gray-400" onclick="event.stopPropagation(); AIPRM.viewPromptTemplateSource(${
                        template.ID
                      })">${svg('Eye')}</a>  
                      <a title="Report this prompt" class="AIPRM__p-1 AIPRM__rounded-md hover:AIPRM__bg-bg-200 hover:AIPRM__text-gray-700 dark:AIPRM__text-gray-400 dark:hover:AIPRM__bg-dark-bg-100 dark:hover:AIPRM__text-gray-200 disabled:dark:hover:AIPRM__text-gray-400" onclick="event.stopPropagation(); AIPRM.showReportPromptModal(${
                        template.ID
                      })">${svg('Report')}</a>    
                      `
                      : ''
                  ...
                  ${
                    template.OwnPrompt || this.isAdminMode()
                      ? /*html*/ `
                    <a title="Delete this prompt" class="AIPRM__p-1 AIPRM__rounded-md hover:AIPRM__bg-bg-200 hover:AIPRM__text-gray-700 dark:AIPRM__text-gray-400 dark:hover:AIPRM__bg-dark-bg-100 dark:hover:AIPRM__text-gray-200 disabled:dark:hover:AIPRM__text-gray-400" onclick="event.stopPropagation(); AIPRM.deletePromptTemplate(${
                      template.ID
                    })">${svg('Trash')}</a>
                    <a title="Edit this prompt" class="AIPRM__p-1 AIPRM__rounded-md hover:AIPRM__bg-bg-200 hover:AIPRM__text-gray-700 dark:AIPRM__text-gray-400 dark:hover:AIPRM__bg-dark-bg-100 dark:hover:AIPRM__text-gray-200 disabled:dark:hover:AIPRM__text-gray-400" onclick="event.stopPropagation(); AIPRM.editPromptTemplate(${
                      template.ID
                    })">${svg('Edit')}</a>
                    `
                      : ''
                  ...
                  ${
                    template.IsHidden
                      ? ''
                      : /*html*/ `
                        <a title="Add this prompt to list" class="AIPRM__p-1 AIPRM__rounded-md hover:AIPRM__bg-bg-200 hover:AIPRM__text-gray-700 dark:AIPRM__text-gray-400 dark:hover:AIPRM__bg-dark-bg-100 dark:hover:AIPRM__text-gray-200 disabled:dark:hover:AIPRM__text-gray-400" onclick="event.stopPropagation(); AIPRM.addToList(${
                          template.ID
                        })">${svg('SquaresPlus')}</a>
                        ...
                        <a title="${`${
                          template.IsFavorite
                            ...
                            : 'Add this prompt to'
                        } Favorites`}" class="AIPRM__p-1 AIPRM__rounded-md hover:AIPRM__bg-bg-200 hover:AIPRM__text-gray-700 dark:AIPRM__text-gray-400 dark:hover:AIPRM__bg-dark-bg-100 dark:hover:AIPRM__text-gray-200 disabled:dark:hover:AIPRM__text-gray-400" onclick="event.stopPropagation(); ${
                          template.IsFavorite
                  ...
                  template.AuthorName
                    ? template.AuthorURL
                      ...
                      <a href="${sanitizeInput(
                        template.AuthorURL
                      )}" class="AIPRM__mx-1 AIPRM__underline AIPRM__overflow-hidden AIPRM__text-ellipsis AIPRM__whitespace-nowrap"
            ...
            <div class="AIPRM__text-gray-500 AIPRM__text-xs AIPRM__flex AIPRM__pt-3 AIPRM__w-full AIPRM__justify-between AIPRM__mt-auto">
                  ...
                  ${
                    !template.OwnPrompt
                      ? /*html*/ `<a title="Votes (${
                        ...
                        class="AIPRM__p-1 AIPRM__rounded-md hover:AIPRM__bg-bg-200 hover:AIPRM__text-gray-700 dark:AIPRM__text-gray-400 dark:hover:AIPRM__bg-dark-bg-100 dark:hover:AIPRM__text-gray-200 disabled:dark:hover:AIPRM__text-gray-400" onclick="event.stopPropagation(); AIPRM.voteThumbsUp(${
                          template.ID
                        })">${svg('ThumbUp')}</a>`
                      : /*html*/ `${svg('ThumbUp')}`
                  }
                  ...
                  ${
                    !template.OwnPrompt
                      ? /*html*/ `&nbsp; <a title="Votes (${
                        ...
                        class="AIPRM__p-1 AIPRM__rounded-md hover:AIPRM__bg-bg-200 hover:AIPRM__text-gray-700 dark:AIPRM__text-gray-400 dark:hover:AIPRM__bg-dark-bg-100 dark:hover:AIPRM__text-gray-200 disabled:dark:hover:AIPRM__text-gray-400" onclick="event.stopPropagation(); AIPRM.voteThumbsDown(${
                          template.ID
                        })">${svg('ThumbDown')}</a>`
                      : ''
                  }
                ...
                <span class="AIPRM__flex AIPRM__items-center" title="Copy link to this prompt">
                  <a class="AIPRM__p-1 AIPRM__rounded-md hover:AIPRM__bg-bg-200 hover:AIPRM__text-gray-700 dark:AIPRM__text-gray-400 dark:hover:AIPRM__bg-dark-bg-100 dark:hover:AIPRM__text-gray-200 disabled:dark:hover:AIPRM__text-gray-400" 
   ...
   `;
    ...
    wrapper.className = `md:AIPRM__flex AIPRM__items-start AIPRM__text-center AIPRM__gap-2.5  AIPRM__m-auto AIPRM__text-sm AIPRM__max-w-full ${
      title.tagName === selectorConfig.DashboardTitleTagName
        ? ' AIPRM__mt-6 '
        : ''
    }`;
          ...
          <form>
            <div
              ...
              <div class="AIPRM__bg-white dark:AIPRM__bg-gray-900 AIPRM__px-4 AIPRM__pt-5 AIPRM__pb-4 sm:AIPRM__p-6 sm:AIPRM__pb-4 AIPRM__w-96">
                ...
                <select id="shareListWithTeamSelectTeam" name="shareListWithTeamSelectTeam" class="AIPRM__select AIPRM__mt-2 AIPRM__mb-3 AIPRM__bg-gray-100 dark:AIPRM__bg-gray-850 dark:AIPRM__border-gray-700 dark:hover:AIPRM__bg-gray-800 AIPRM__rounded AIPRM__w-full AIPRM__px-3 AIPRM__py-2">
                  ${
  ...
  insertLanguageToneWritingStyleContinueActions() {
    ...
    // Set parent to be the textarea
    const parent = textarea;
  ...
  insertIncludeMyProfileInfo() {
    ...
    let wrapperClassName =
      'AIPRM__pl-10 AIPRM__text-right AIPRM__whitespace-nowrap';
    ...
    // Insert the checkbox
    wrapper.innerHTML = /*html*/ `
      <label class="AIPRM__inline-flex AIPRM__text-sm AIPRM__items-center AIPRM__mx-2"
        title="Include provided &quot;My Profile&quot; info that you would like Claude to know and remember about you and your preferences.">
      ...
      this.MyProfileInfos.length > 0 ? '' : 'disabled'
        ...
        <span>Include <a class="AIPRM__underline AIPRM__cursor-pointer" title="Manage My Profile Info" href="${AppAccountURL}#myprofileinfo" target="_blank">My Profile Info</a>:</span>
      </label>

      <div id="includeMyProfileInfoSelectWrapper" class="AIPRM__inline-block AIPRM__group">
        <select id="includeMyProfileInfoSelect" class="AIPRM__select AIPRM__bg-bg-300 AIPRM__border-0 AIPRM__text-sm AIPRM__rounded dark:AIPRM__bg-dark-bg-200 dark:AIPRM__border-gray-600 dark:group-hover:AIPRM__bg-dark-bg-300 dark:AIPRM__placeholder-gray-400 dark:AIPRM__text-white group-hover:AIPRM__bg-bg-400 focus:AIPRM__ring-0 sm:AIPRM__max-w-xs AIPRM__max-w-[6rem] AIPRM__px-3 AIPRM__py-2 ${
  ...
  insertVariablesInputWrapper() {
    ...
    if (variableWrapper) {
      ...
      variableWrapper.className =
        'AIPRM__gap-3 lg:AIPRM__max-w-3xl md:last:AIPRM__mb-6 AIPRM__mx-2 AIPRM__pt-2 AIPRM__stretch AIPRM__justify-between AIPRM__text-sm AIPRM__pb-2 AIPRM__mb-2 AIPRM__border-b AIPRM__border-black/10 AIPRM__hidden';
  ...
  // Ask Claude to continue writing
  continueWriting() {
    this.submitContinueActionPrompt('Continue writing please');
  },

  // Submit the continue action prompt to Claude
  submitContinueActionPrompt(prompt = '') {
    ...
    if (!textarea) {
      console.error('submitContinueActionPrompt: No textarea found');
      return;
    }
    ...
    if (
      ...
      input.innerText.trim() !== 'Continue writing please' &&
      !confirm(
  ...
  async copyPromptDeepLink(idx) {
    ...
    const promptLink =
      prompt.PromptTypeNo === PromptTypeNo.PUBLIC
        ...
        : `https://claude.ai/new?${queryParamPromptID}=${prompt.ID}`;
      ...
      wrapper.innerHTML = /*html*/ `
        <span class="${css`tag`}" title="${sanitizeInput(template.Teaser)}">
          ...
          <a title="Deselect prompt / New chat"
            class="AIPRM__cursor-pointer AIPRM__ml-3 AIPRM__align-middle AIPRM__inline-block AIPRM__rounded-md dark:AIPRM__text-gray-400 hover:AIPRM__bg-dark-bg-0 hover:AIPRM__text-gray-200 disabled:dark:hover:AIPRM__text-gray-400" 
          ...
          if (promptVariable.EnumS.length > 0) {
            variableHTML += /*html*/ `
              <select id="${variableIDPrefix}${promptVariable.ID}" 
                ...
                class="AIPRM__select AIPRM__w-full AIPRM__border-0 AIPRM__rounded AIPRM__p-2 AIPRM__mt-1 AIPRM__bg-bg-300 dark:AIPRM__bg-dark-bg-200 dark:AIPRM__border-gray-850 dark:hover:AIPRM__bg-dark-bg-300 dark:AIPRM__placeholder-gray-400 dark:AIPRM__text-white hover:AIPRM__bg-bg-400" required>
           ...
           `;
            ...
            variableHTML += /*html*/ `
              ...
              class="AIPRM__w-full AIPRM__border-0 AIPRM__rounded AIPRM__p-2 AIPRM__mt-1 AIPRM__bg-bg-300 dark:AIPRM__bg-dark-bg-200 dark:AIPRM__border-gray-850 dark:hover:AIPRM__bg-dark-bg-300 dark:AIPRM__placeholder-gray-400 dark:AIPRM__text-white hover:AIPRM__bg-bg-400">${sanitizeInput(
              ...
              role="dialog" aria-modal="true" aria-labelledby="modal-headline">
                ...
                <div class="AIPRM__bg-white dark:AIPRM__bg-gray-900 AIPRM__px-4 AIPRM__pt-5 AIPRM__pb-4 sm:AIPRM__p-6 sm:AIPRM__pb-4 AIPRM__overflow-y-auto">
                  ...
                  <div class="AIPRM__flex">
                    <div class="AIPRM__mr-4 AIPRM__w-full">
                      <label>Topic</label>
                      <select disabled name="Topic" class="AIPRM__select AIPRM__mt-2 AIPRM__mb-3 AIPRM__bg-gray-100 dark:AIPRM__bg-gray-850 dark:AIPRM__border-gray-700 dark:hover:AIPRM__bg-gray-800 AIPRM__rounded AIPRM__w-full AIPRM__px-3 AIPRM__py-2" required>
                        ${this.Topics.map(
                          (topic) => /*html*/ `
                              <option value="${sanitizeInput(topic.ID)}" ${
                    ...
                    <div class="AIPRM__w-full">
                      <label>Activity</label>
                      <select disabled name="Activity" class="AIPRM__select AIPRM__mt-2 AIPRM__mb-3 AIPRM__bg-gray-100 dark:AIPRM__bg-gray-850 dark:AIPRM__border-gray-700 dark:hover:AIPRM__bg-gray-800 AIPRM__rounded AIPRM__w-full AIPRM__px-3 AIPRM__py-2" required>
  ...
  showListSelectionModal(lists, prompt, onlyTeamLists = false) {
    ...
    listSelectionModal.innerHTML = /*html*/ `
      ...
      onlyTeamLists ? 'Choose Team List to add to' : 'Choose a list'
                  ...
                  <select name="listID" class="AIPRM__select AIPRM__mt-2 AIPRM__mb-3 AIPRM__bg-gray-100 dark:AIPRM__bg-gray-850 dark:AIPRM__border-gray-700 dark:hover:AIPRM__bg-gray-800 AIPRM__rounded AIPRM__w-full AIPRM__px-3 AIPRM__py-2">
                    ${lists
                      .map((list) => {
                        if (
                  ...
                  <div id="createNewList" class="${
                    ...
                    <input type="text" name="listName" class="AIPRM__mt-2 AIPRM__mb-3 AIPRM__bg-gray-100 dark:AIPRM__bg-gray-850 dark:AIPRM__border-gray-700 dark:hover:AIPRM__bg-gray-800 AIPRM__rounded AIPRM__w-full AIPRM__p-2" ${
                      !(
                        lists.length ||
                        (prompt.PromptTypeNo !== PromptTypeNo.PRIVATE &&
                          this.Lists.withType(ListTypeNo.CUSTOM) !== undefined)
                    ...
                    ${
                      ...
                      this.Client.UserQuota?.hasTeamsFeatureEnabled()
                        ...
                        <div id="createNewListSelectTeam" class="${
                          ...
                          <select id="createNewListSelectTeam" name="createNewListSelectTeam" class="AIPRM__select AIPRM__mt-2 AIPRM__mb-3 AIPRM__bg-gray-100 dark:AIPRM__bg-gray-850 dark:AIPRM__border-gray-700 dark:hover:AIPRM__bg-gray-800 AIPRM__rounded AIPRM__w-full AIPRM__px-3 AIPRM__py-2">
                            ${
                              this.Client.OwnTeamS?.length > 0
                                ? this.Client.OwnTeamS.map(
                                    (team) =>
              ...
              <div
                ...
                <div class="AIPRM__bg-white dark:AIPRM__bg-gray-900 AIPRM__px-4 AIPRM__pt-5 AIPRM__pb-4 sm:AIPRM__p-6 sm:AIPRM__pb-4 AIPRM__w-96">
                  ...
                  <h3 class="${css`h3`} AIPRM__my-4">Create a new list</h3>
                    ...
                    <label class="AIPRM__block">List Name</label>
                    <input type="text" name="listName" class="AIPRM__mt-2 AIPRM__mb-3 AIPRM__bg-gray-100 dark:AIPRM__bg-gray-850 dark:AIPRM__border-gray-700 dark:hover:AIPRM__bg-gray-800 AIPRM__rounded AIPRM__w-full AIPRM__p-2" required>
  ...
  // Show AIPRM Account modal (connect/disconnect AIPRM account, current AIPRM Plan level and Claude account)
  showAccountModal() {
              ...
              role="dialog" aria-modal="true" aria-labelledby="modal-headline">
                ...
                <div class="AIPRM__bg-white dark:AIPRM__bg-gray-900 AIPRM__p-4 AIPRM__px-6 AIPRM__overflow-y-auto">
                  ...
                  <dl class="AIPRM__text-sm">
                      ...
                      <dd class="AIPRM__w-full sm:AIPRM__w-1/2 AIPRM__py-2 AIPRM__justify-between">
                        ...
                        ${
                          this.Client.User.IsLinked
                            ...
                            `
                            : /*html*/ `
                              <a class="AIPRM__inline-block AIPRM__mt-4 AIPRM__bg-white AIPRM__border AIPRM__border-green-500 hover:AIPRM__border-green-700 AIPRM__text-green-500 hover:AIPRM__text-green-700 AIPRM__py-2 AIPRM__px-3 AIPRM__rounded dark:AIPRM__bg-gray-900 dark:hover:AIPRM__text-green-400 dark:hover:AIPRM__border-green-400" href="${AppAccountURL}?action=connectClaude" target="_blank">
                                Connect
                              </a>
                            `
                        }
                    ...
                    <div class="AIPRM__flex AIPRM__py-4 AIPRM__flex-col sm:AIPRM__flex-row AIPRM__border-b AIPRM__border-black/10 dark:AIPRM__border-gray-700">
                      <dt class="AIPRM__w-full sm:AIPRM__w-1/2 AIPRM__py-2 AIPRM__font-medium">Claude Account</dt>
                      <dd class="AIPRM__w-full sm:AIPRM__w-1/2 AIPRM__py-2 AIPRM__justify-between">
                        <div>
                           ${sanitizeInput(
                             this.Client.ClaudeUser?.Email || 'Unknown'
                           )}
                        </div>
                      </dd>
                    </div>
  ...
  connectAccount() {
    ...
    // open new window
    window.open(`${AppAccountURL}?action=connectClaude`, '_blank');
  },
  ...
  prefillPromptInput() {
    ...
    if (!input) {
      console.error('prefillPromptInput: No input found');
      return;
    }
  ...
  async modifyConversationMessages(fetchConversationMessages) {
    ...
    const userMessages = [
      ...document.querySelectorAll(selectorConfig.ConversationUserMessages),
    ]
      // Find name of the current __reactFiber* property first (random/unknown) and then get the uuid for message
      ?.map((el) => {
        const reactFiber = Object.keys(el).find((key) =>
          key?.includes(selectorConfig.ReactFiberPropertyKey)
        );
        return {
          uuid: el?.[reactFiber]?.return?.memoizedProps?.message?.uuid || null,
          text:
            el?.[reactFiber]?.return?.memoizedProps?.message?.text ||
            el?.[reactFiber]?.return?.memoizedProps?.message?.content?.[0]
              ?.text ||
            '',
        };
      });
    ...
    conversation.chat_messages = conversation?.chat_messages?.map((message) => {
      if (message?.sender !== selectorConfig.MessageSenderUser) {
        return message;
      }

      // Find user message using uuid
      let userMessage = userMessages?.find(
        (userMessage) => userMessage?.uuid === message?.uuid
      );

      // Try to find "new-human-message-uuid" uuid user message as a fallback (new message)
      if (!userMessage) {
        // console.log(
        //   'User message not found, trying to find new-human-message-uuid',
        //   message
        // );

        userMessage = userMessages?.find(
          (userMessage) =>
            userMessage?.uuid === selectorConfig.NewUserMessageUUID
        );
      }

      // If user message found, add the text to the message
      if (userMessage) {
        ...
        // Update also content.0.text if type is text (chat messages)
        if (message.content?.[0]?.type === 'text') {
          message.content[0].text = userMessage.text;
        }
      } else {
        // console.log('User message not found for message:', message);
      }
...
// hook into pushState & replaceState to detect login flow and re-init AIPRM after login
const pushState = history.pushState;

history.pushState = function () {
  const newURL = arguments[2]?.toString();
  const currentURL = window.location.pathname;

  pushState.apply(this, arguments);

  // re-init AIPRM after login flow
  if (
    (newURL?.startsWith('/new') || newURL === '/') &&
    (currentURL?.startsWith('/login') ||
      currentURL?.startsWith('/magic-link')) &&
    !window.AIPRM?.Client?.ClaudeUser?.OperatorERID
  ) {
    window.AIPRM.init();
  }
};
